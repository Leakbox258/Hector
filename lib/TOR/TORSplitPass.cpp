...
struct TimeEdge {
  ...
  TimeEdge(int _from, int _to, Attribute _attr, bool _static = true,
           bool _exit = false, bool _pipe = false) {
    from = _from;
    to = _to;
    attr = _attr;
    loop_exit = _exit;
...
struct SplitSchedule : public OpRewritePattern<tor::FuncOp> {
  SplitSchedule(MLIRContext *context)
      : OpRewritePattern<tor::FuncOp>(context, 1) {}

  LogicalResult matchAndRewrite(tor::FuncOp funcOp,
                                PatternRewriter &rewriter) const override {
    // reject if strategy is not "mixed"
...
#undef BIND
      if (auto ifop = dyn_cast<tor::IfOp>(op)) {
        ...
        //                        std::cerr << ":," << forop.starttime() <<
        //                        forop.endtime() << std::endl;
      }
    });
    //                exit(0);
    ...
    int visitMark[TIME_NODE * TIME_NODE], visitMarkCount = 0,
                                          nodeMark[TIME_NODE];
    ...
    auto outline = [&](int start, int end, bool pipe = false) {
      outline_ops.clear();
      std::function<void(int, std::function<void(Operation * op)>)>
          visitOperation = [&](int start,
                               std::function<void(Operation * op)> insert) {
            if (start == end)
              return;
            std::cerr << "~~" << start << " " << end << std::endl;
            ...
            if (forEnd[start] != -1 &&
                (end == forEnd[start] || !connected(end, forEnd[start]))) {
              insert(forOp[start]);
      ...
      visitOperation(start, [&](Operation *op) {
        op->walk([&](Operation *op) { outline_ops.insert(op); });
      });
      struct Liveness {
        Value val;
      ...
      for (auto op : outline_ops) {
        ...
        for (auto val : op->getResults()) {
          for (auto it = val.getUses().begin(); it != val.getUses().end();
               ++it) {
            auto bop = it.getUser();
        ...
        for (unsigned idx = 0; idx != op->getNumOperands(); ++idx) {
          ...
          if (auto arg = llvm::dyn_cast<BlockArgument>(val)) {
            arg.dump();
            std::cerr << "~~~";
            if (outline_ops.find(arg.getOwner()->getParentOp()) !=
                outline_ops.end()) {
              continue;
            }
      ...
      if (pipe) {
        ...
        for (auto val : liveins) {
          if (val == forOp.lowerBound())
            continue;
          if (val == forOp.upperBound())
            continue;
          if (val == forOp.step())
            continue;
      ...
      auto out_func =
          rewriter.create<tor::FuncOp>(funcOp.getLoc(), func_name, func_type);
      out_func->setAttr("strategy", StringAttr::get(getContext(), "static"));
      SmallVector<Location, 8> argLocs(argTypes.size(), funcOp.getLoc());
      Block *outBlock =
          rewriter.createBlock(&(out_func.getBody()), {}, argTypes, argLocs);
      IRMapping mapper;
      for (auto val : liveins) {
        auto it = argNum.find(val);
        if (it != argNum.end())
          mapper.map(val.val, outBlock->getArgument(it->second));
      }
      ...
      /*for (unsigned idx = 0; idx < argValues.size(); ++idx) {
          ...
          if (lastOp->getBlock() == newOp->getBlock()) {
              ...
              if
      (lastOp->isBeforeInBlock(lastOp->getBlock()->findAncestorOpInBlock(*newOp)))
      { lastOp = newOp;
              }
          }
      }*/
      visitOperation(start, [&](Operation *op) {
        ...
        if (lastOp->getBlock() == op->getBlock()) {
          ...
          if (!lastOp->isBeforeInBlock(
                  lastOp->getBlock()->findAncestorOpInBlock(*op))) {
            lastOp = op;
          }
        }
      });
      ...
      auto callOp =
          rewriter.create<tor::CallOp>(lastOp->getLoc(), retTypes, func_name,
                                       start, end, ValueRange(argValues));
      ...
      visitOperation(start, [&](Operation *op) {
        ...
        for (auto val : op->getResults()) {
          if (liveouts.find(val) != liveouts.end()) {
            ...
            std::cerr << "---------REPLACE---------" << val.getResultNumber()
                      << std::endl;
            val.dump();
            ...
            for (auto &use : use_vec) {
              ...
              val.replaceUsesWithIf(callOp.getResult(retnum),
                                    [&](OpOperand &operand) {
        ...
        //                            val->dump();
        //                        }
        //                        std::cerr << "111111111111111111111" <<
        //                        std::endl; std::cerr << op->use_empty(); for
        //                        (auto &test : op->getUses()) {
        //                            test.drop();
        //                        }
        //                        op->dropAllUses();
        //                        std::cerr << "222222222222222222222" <<
        //                        std::endl;
        rewriter.eraseOp(op);
        //                        std::cerr << "333333333333333333333" <<
        //                        std::endl;
      ...
      out_func.walk([&](Operation *op) {
        for (unsigned idx = 0; idx != op->getNumOperands(); ++idx) {
          ...
          auto argIt = argNum.find(val);
          if (argIt != argNum.end()) {
            op->setOperand(idx, out_func.getArgument(argIt->second));
          }
          continue;
          if (!bop) {
            if (auto arg = llvm::dyn_cast<BlockArgument>(val)) {
          ...
          for (auto bval : bop->getResults()) {
            if (bval == val) {
              if (liveins.find(bval) != liveins.end()) {
                op->setOperand(idx, out_func.getArgument(argNum[bval]));
              }
            }
          }
        }
      ...
      auto new_timegraph =
          rewriter.create<tor::TimeGraphOp>(out_func.getLoc(), start, end);
      rewriter.createBlock(&(new_timegraph.region()));
      rewriter.setInsertionPointToStart(new_timegraph.getBody());
      //                    rewriter.create<tor::StartTimeOp>(out_func.getLoc(),
      //                    start);
      eraseEdges.clear();
      ++visitMarkCount;
      ...
      auto newTimeEdge = [&](TimeEdge *edge) {
        ...
        for (size_t j = 0; j < succ.points().size(); j++) {
          if (llvm::cast<IntegerAttr>(succ.points()[j]).getInt() !=
              edge->from) {
      ...
      for (auto &edge : eraseEdges) {
        froms[edge->to].push_back(
            IntegerAttr::get(mlir::IntegerType::get(
                                 getContext(), 32, mlir::IntegerType::Signless),
                             edge->from));
      ...
      dict_attr.push_back(
          NamedAttribute(StringAttr::get(getContext(), "type"),
      ...
      auto newSuccOp = rewriter.create<tor::SuccTimeOp>(
          succOp[end]->getLoc(), end, ArrayAttr::get(getContext(), new_from),
    ...
    std::function<void(int)> foundStaticPart = [&](int start) {
      ...
      int count =
          std::count_if(timeGraph[start].begin(), timeGraph[start].end(),
                        [&](const TimeEdge &edge) { return !edge.loop_exit; });
      std::cerr << start << ":::" << count << std::endl;
      if (count == 1) {
        int end = start;
        while (1) {
          int count = std::count_if(
              timeGraph[end].begin(), timeGraph[end].end(),
              [&](const TimeEdge &edge) { return !edge.loop_exit; });
          if (count == 0) {
            break;
          } else if (count == 1) {
...
std::unique_ptr<mlir::OperationPass<mlir::ModuleOp>> createTORSplitPass() {
