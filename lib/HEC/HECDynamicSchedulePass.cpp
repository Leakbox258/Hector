...
struct Liveness {
  ...
  Liveness(const Value &_val) { val = _val; }

  Liveness(const Liveness &live) { val = live.val; }
  ...
  bool operator==(const Liveness &x) const { return val == x.val; }

  operator bool() const { return val != control_signal; }
  ...
  if (auto forOp = dyn_cast<tor::ForOp>(op)) {
    ...
    for (auto &op : forOp.getRegion().front()) {
      ...
      for (auto &val : liveins[&op]) {
        ...
        if (!llvm::isa<BlockArgument>(val.val) &&
            isa<ConstantOp>(val.val.getDefiningOp())) {
          continue;
        }
    ...
    for (unsigned idx = 0; idx < ifOp->getNumRegions(); ++idx) {
      ...
      for (auto val :
           ifOp.getRegion(idx).front().getTerminator()->getOperands()) {
        liveout.insert(val);
        if (update_liveout(val.getDefiningOp(), liveout)) {
          flag = true;
        }
...
hec::PrimitiveOp create_primitive(Location loc,
                                  llvm::SmallVector<mlir::Type, 4> &types,
                                  std::string primitive, std::string name,
                                  PatternRewriter &rewriter) {
  auto lastInsertion = rewriter.saveInsertionPoint();
  rewriter.setInsertionPoint(TopComp->getGraphOp());
  auto context = rewriter.getContext();
  auto instanceName =
      StringAttr::get(context, llvm::StringRef(name + std::to_string(count++)));
  auto primitiveName = StringAttr::get(context, primitive);
  auto primOp = rewriter.create<hec::PrimitiveOp>(loc, mlir::TypeRange(types),
                                                  instanceName, primitiveName);
...
Value get_value(Operation *op, Value val, PatternRewriter &rewriter) {
  if (val != control_signal && val.getDefiningOp()) {
    ...
    if (isa<ConstantOp>(sop)) {
      ...
      //                    if (hec_operation.find(std::make_pair(op,
      //                    Liveness(val))) == hec_operation.end())
      if (hec_operation[std::make_pair(op, Liveness(val))] == Value()) {
        ...
        types.push_back(sop->getResult(0).getType());
        auto constant = create_primitive(op->getLoc(), types, "constant",
                                         "const_", rewriter);
        hec_operation[std::make_pair(op, Liveness(val))] =
            constant.getResult(2);
        rewriter.create<hec::AssignOp>(
            op->getLoc(), constant.getResult(0),
            get_value(op->getParentOp(), control_signal, rewriter), Value());
        rewriter.create<hec::AssignOp>(op->getLoc(), constant.getResult(1),
                                       new_operation[sop]->getResult(0),
                                       Value());
        //                        constant->dump();
      } else {
        //                        std::cerr << "FOUND\n";
        //                        hec_operation[std::make_pair(op,
        //                        Liveness(val))].dump(); std::cerr <<
        //                        (hec_operation[std::make_pair(op,
        //                        Liveness(val))] == Value()); std::cerr <<
        //                        "FOUND\n"; exit(-1);
      }
...
void Generate_operation(Operation *op, PatternRewriter &rewriter) {
  ...
  for (auto val : liveins[op]) {
    ...
    hec_operation[std::make_pair(op, val)] =
        hec_operation[std::make_pair(op->getParentOp(), val)];
  }
  if (auto forOp = dyn_cast<tor::ForOp>(op)) {
    ...
    compare = create_primitive(forOp.getLoc(), types, "cmp_integer_sle",
                               "cmpi_sle_", rewriter);
    ...
    types.push_back(rewriter.getI1Type());
    auto c_merge = create_primitive(forOp.getLoc(), types, "control_merge",
                                    "m_", rewriter);
    types.clear();
    ...
    auto buffer =
        create_primitive(forOp.getLoc(), types, "buffer", "buf_", rewriter);
    types.clear();
    types.push_back(rewriter.getI1Type());
    for (int loop = 0; loop < 3; ++loop) {
      types.push_back(rewriter.getIntegerType(1000));
    ...
    auto branch =
        create_primitive(forOp.getLoc(), types, "branch", "b_", rewriter);
    rewriter.create<hec::AssignOp>(forOp.getLoc(), branch.getResult(1),
                                   buffer.getResult(1), Value());
    rewriter.create<hec::AssignOp>(forOp.getLoc(), buffer.getResult(0),
                                   c_merge.getResult(2), Value());
    rewriter.create<hec::AssignOp>(forOp.getLoc(), branch.getResult(0),
                                   compare.getResult(2), Value());
    rewriter.create<hec::AssignOp>(forOp.getLoc(), c_merge.getResult(0),
                                   get_value(forOp, control_val.val, rewriter),
                                   Value());
    ...
    for (auto arg : forOp.getBody()->getArguments()) {
      ...
      types.push_back(rewriter.getI1Type());
      auto mux = create_primitive(forOp.getLoc(), types, "mux_dynamic", "m_",
                                  rewriter);
      ...
      rewriter.create<hec::AssignOp>(forOp.getLoc(), branch.getResult(0),
                                     compare.getResult(2), Value());
      rewriter.create<hec::AssignOp>(
          forOp.getLoc(), mux.getResult(0),
          get_value(forOp, forOp.initArgs()[arg.getArgNumber() - 1], rewriter),
          Value());
      rewriter.create<hec::AssignOp>(forOp.getLoc(), mux.getResult(3),
                                     merge_condition, Value());
...
#undef LIVE_INSERT
    ...
    for (auto val : liveins[forOp]) {
      ...
      rewriter.create<hec::AssignOp>(forOp.getLoc(), mux.getResult(0),
                                     get_value(forOp, val.val, rewriter),
                                     Value());
      rewriter.create<hec::AssignOp>(forOp.getLoc(), mux.getResult(1),
                                     branch.getResult(2), Value());
      rewriter.create<hec::AssignOp>(forOp.getLoc(), mux.getResult(3),
                                     merge_condition, Value());
    ...
    for (auto arg : forOp.getBody()->getArguments()) {
      ...
      types.push_back(forOp.upperBound().getType());
      auto add = create_primitive(forOp.getLoc(), types, "add_integer", "addi_",
                                  rewriter);
      hec_operation[std::make_pair(forOp, arg)] = branch.getResult(2);

      rewriter.create<hec::AssignOp>(forOp.getLoc(), mux.getResult(0),
                                     lowerBound, Value());
      rewriter.create<hec::AssignOp>(forOp.getLoc(), compare.getResult(0),
                                     buffer.getResult(1), Value());
      rewriter.create<hec::AssignOp>(forOp.getLoc(), compare.getResult(1),
                                     upperBound, Value());
      rewriter.create<hec::AssignOp>(forOp.getLoc(), branch.getResult(0),
                                     compare.getResult(2), Value());
      rewriter.create<hec::AssignOp>(forOp.getLoc(), branch.getResult(1),
                                     buffer.getResult(1), Value());
      rewriter.create<hec::AssignOp>(forOp.getLoc(), add.getResult(0),
                                     branch.getResult(2), Value());
      rewriter.create<hec::AssignOp>(forOp.getLoc(), add.getResult(1),
                                     get_value(forOp, forOp.step(), rewriter),
                                     Value());
      rewriter.create<hec::AssignOp>(forOp.getLoc(), mux.getResult(1),
                                     add.getResult(2), Value());
      rewriter.create<hec::AssignOp>(forOp.getLoc(), mux.getResult(3),
                                     merge_condition, Value());
    }
      ...
      for (auto pair : hec_operation) {
        if (pair.first.first->getParentOp() == forOp) {
          if (!pair.first.second) {
            ...
            if (isa<tor::ForOp>(op)) {
              hec_operation[std::make_pair(forOp, pair.first.second)] =
                  pair.second;
            }
            continue;
          }
    ...
    for (unsigned idx = 0; idx < yield->getNumOperands(); ++idx) {
      ...
      rewriter.create<hec::AssignOp>(forOp.getLoc(),
                                     mux_set[idx + 1]->getResult(1),
                                     get_value(forOp, ret, rewriter), Value());
      hec_operation[std::make_pair(forOp, forOp->getResult(idx))] =
          branch_set[idx + 1]->getResult(3);
    }
    ...
    for (auto val : liveins[ifOp]) {
      ...
      types.push_back(rewriter.getI1Type());
      for (int loop = 0; loop < 3; ++loop) {
        types.push_back(val ? val.val.getType()
                            : rewriter.getIntegerType(1000));
      }
      auto branch =
          create_primitive(ifOp.getLoc(), types, "branch", "b_", rewriter);
      branch_set.push_back(branch);
      rewriter.create<hec::AssignOp>(
          ifOp.getLoc(), branch->getResult(0),
          get_value(ifOp, ifOp.condition(), rewriter), Value());
      rewriter.create<hec::AssignOp>(ifOp.getLoc(), branch->getResult(1),
                                     get_value(ifOp, val.val, rewriter),
                                     Value());
    }
    for (auto ret : ifOp->getResults()) {
      ...
      types.push_back(rewriter.getI1Type());
      auto cmerge =
          create_primitive(ifOp.getLoc(), types, "mux_dynamic", "m_", rewriter);
      mux_set.push_back(cmerge);
      hec_operation[std::make_pair(ifOp, ret)] = cmerge.getResult(2);
      rewriter.create<hec::AssignOp>(
          ifOp.getLoc(), cmerge.getResult(3),
          get_value(ifOp, ifOp.condition(), rewriter), Value());
    }
    for (unsigned idx = 0; idx < ifOp->getNumRegions(); ++idx) {
      int branch_count = 0;
      for (auto val : liveins[ifOp]) {
        ...
        hec_operation[std::make_pair(ifOp, val.val)] =
            branch_set[branch_count++]->getResult(idx + 2);
      }
      if (ifOp.getRegion(idx).empty()) {
        continue;
      }
      if (!ifOp.getRegion(idx).empty()) {
        for (auto &op : *(ifOp.getBody(idx))) {
          ...
          Generate_operation(&op, rewriter);
          for (auto pair : hec_operation) {
            if (pair.first.first == &op &&
                pair.first.first->getParentOp() == ifOp) {
              if (!pair.first.second) {
                continue;
              }
              if (llvm::isa<BlockArgument>(pair.first.second.val)) {
              ...
              /*
                                              if
                 (pair.first.second.val.getDefiningOp()->getParentOp() != ifOp)
                 { continue;
                                              }
              */
              if (pair.first.second.val.getDefiningOp() != pair.first.first) {
              ...
              hec_operation[std::make_pair(ifOp, pair.first.second)] =
                  pair.second;
            }
          }
        }
      ...
      for (unsigned yield_cnt = 0; yield_cnt < yield->getNumOperands();
           ++yield_cnt) {
        auto ret = yield->getOperand(yield_cnt);
        //                        std::cerr << idx << "::";
        rewriter.create<hec::AssignOp>(ifOp.getLoc(),
                                       mux_set[yield_cnt]->getResult(!idx),
    ...
    auto instance = rewriter.create<hec::InstanceOp>(
        op->getLoc(), mlir::TypeRange(types),
        callee.str() + "_" + std::to_string(count), callee);
    rewriter.restoreInsertionPoint(lastInsertion);
    for (auto ret : callOp->getResults()) {
      llvm::SmallVector<mlir::Type, 4> types;
      types.push_back(instance
                          .getResult(callOp.getArgOperands().size() + 1 +
                                     ret.getResultNumber())
                          .getType());
      types.push_back(instance
                          .getResult(callOp.getArgOperands().size() + 1 +
                          ...
                          .getType());
      auto fifo =
          create_primitive(op->getLoc(), types, "fifo:1", "fifo_", rewriter);
      rewriter.create<hec::AssignOp>(
          op->getLoc(), fifo.getResult(0),
          instance.getResult(callOp.getArgOperands().size() + 1 +
                             ret.getResultNumber()),
          Value());
      hec_operation[std::make_pair(op, ret)] = fifo.getResult(1);
      //                    hec_operation[std::make_pair(op, ret)] =
      //                    instance.getResult(
      //                            callOp.getArgOperands().size() + 1 +
      //                            ret.getResultNumber());
    }
    //                TopComp->dump();
    instance->dump();
    instance.getReferencedComponent()->setAttr(
        "interfc", StringAttr::get(rewriter.getContext(), "wrapped"));
    // FIXME: void function needs control_signal
    for (unsigned idx = 0; idx < callOp.getArgOperands().size(); ++idx) {
      rewriter.create<hec::AssignOp>(
          callOp.getLoc(), instance->getResult(idx),
    ...
    for (unsigned idx = 0; idx != vec.size(); ++idx) {
      ...
      if (op == loadOp) {
        rewriter.create<hec::AssignOp>(loadOp.getLoc(),
                                       memOp->getResult(2 * idx),
                                       load.getResult(2), Value());
    ...
    for (unsigned idx = 0; idx != vec.size(); ++idx) {
      auto const &op = vec[idx];
      if (op == storeOp) {
        rewriter.create<hec::AssignOp>(storeOp.getLoc(),
                                       memOp->getResult(2 * (idx + loadSize)),
                                       store.getResult(2), Value());
        rewriter.create<hec::AssignOp>(
            storeOp.getLoc(), memOp->getResult(2 * (idx + loadSize) + 1),
...
#define CREATE_PRIMITIVE(TYPE, PRIMITIVE, NAME)                                \
  if (auto tor_op = dyn_cast<TYPE>(op)) {                                      \
    ...
    auto primitive =                                                           \
        create_primitive(op->getLoc(), types, PRIMITIVE, NAME, rewriter);      \
    int retNum = tor_op->getNumOperands();                                     \
    for (auto ret : tor_op->getResults()) {                                    \
      hec_operation[std::make_pair(tor_op, Liveness(ret))] =                   \
          primitive.getResult(retNum++);                                       \
    }                                                                          \
    int primitive_count = 0;                                                   \
    for (auto val : tor_op->getOperands()) {                                   \
      rewriter.create<hec::AssignOp>(tor_op.getLoc(),                          \
                                     primitive.getResult(primitive_count++),   \
                                     get_value(op, val, rewriter), Value());   \
    }                                                                          \
  }
    CREATE_PRIMITIVE(tor::AddIOp, "add_integer", "addi_")
    else CREATE_PRIMITIVE(tor::SubIOp,
                          "sub_integer", "subi_") else CREATE_PRIMITIVE(tor::MulIOp, "mul_integer", "muli_") else CREATE_PRIMITIVE(tor::
                                                                                                                                       AddFOp,
                                                                                                                                   "add_float",
                                                                                                                                   "addf_") else CREATE_PRIMITIVE(tor::
                                                                                                                                                                      SubFOp,
                                                                                                                                                                  "sub_float", "subf_") else CREATE_PRIMITIVE(tor::MulFOp, "mul_float", "mulf_") else CREATE_PRIMITIVE(tor::
                                                                                                                                                                                                                                                                           DivFOp,
                                                                                                                                                                                                                                                                       "div_float", "divf_") else CREATE_PRIMITIVE(TruncateIOp, "trunc_integer", "trunci_") else CREATE_PRIMITIVE(tor::
                                                                                                                                                                                                                                                                                                                                                                                      CmpIOp,
                                                                                                                                                                                                                                                                                                                                                                                  std::string("cmp_integer_") + tor::stringifyEnum(
                                                                                                                                                                                                                                                                                                                                                                                                                    tor_op
                                                                                                                                                                                                                                                                                                                                                                                                                        .getPredicate())
                                                                                                                                                                                                                                                                                                                                                                                                                    .str(),
                                                                                                                                                                                                                                                                                                                                                                                  "cmpi_") else CREATE_PRIMITIVE(tor::
                                                                                                                                                                                                                                                                                                                                                                                                                     CmpFOp,
                                                                                                                                                                                                                                                                                                                                                                                                                 std::string("cmp_float_") + tor::stringifyEnum(
                                                                                                                                                                                                                                                                                                                                                                                                                                                 tor_op
                                                                                                                                                                                                                                                                                                                                                                                                                                                     .getPredicate())
                                                                                                                                                                                                                                                                                                                                                                                                                                                 .str(),
                                                                                                                                                                                                                                                                                                                                                                                                                 "cmpf_") else CREATE_PRIMITIVE(ShiftLeftOp,
                                                                                                                                                                                                                                                                                                                                                                                                                                                "shift_left", "shr_") else CREATE_PRIMITIVE(SelectOp,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            "select",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            "select_") else CREATE_PRIMITIVE(NegFOp, "neg_float", "negf_") else CREATE_PRIMITIVE(AndOp,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 "and", "and_") else CREATE_PRIMITIVE(FPToSIOp,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      "fptosi",
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      "fptosi_") else {
      op->dump();
...
void insert_fifo(hec::ComponentOp &component, PatternRewriter &rewriter) {
  ...
  auto getLatency = [&](Operation *op) {
    if (auto primitive = dyn_cast<hec::PrimitiveOp>(op)) {
      ...
      int latency = llvm::cast<IntegerAttr>(
                        instanceOp.getReferencedComponent()->getAttr("latency"))
                        .getInt();
      return latency;
    }
    return 0;
  ...
  for (auto &op : *(component.getGraphOp().getBody())) {
    if (auto assignOp = dyn_cast<hec::AssignOp>(op)) {
      ...
      graph[dest.getDefiningOp()].push_back(
          std::make_pair(src.getDefiningOp(), &op));
    }
  }
  ...
  for (const auto &pair : time) {
    ...
    for (auto pred : graph[pair.first]) {
      ...
      if (temp != 0) {
        ...
        auto fifo =
            create_primitive(component.getLoc(), types,
                             "fifo:" + std::to_string(temp), "fifo_", rewriter);
        rewriter.create<hec::AssignOp>(component.getLoc(), fifo.getResult(0),
                                       src, Value());
        rewriter.create<hec::AssignOp>(component.getLoc(), dest,
                                       fifo.getResult(1), Value());
        //                        component->dump();
...
void Generate_operation(tor::FuncOp funcOp, PatternRewriter &rewriter) {
  ...
  ports.push_back(hec::ComponentPortInfo(mlir::StringAttr::get(context, "in"),
                                         rewriter.getIntegerType(1000),
  ...
  ports.push_back(hec::ComponentPortInfo(mlir::StringAttr::get(context, "out"),
                                         rewriter.getIntegerType(1000),
                                         hec::PortDirection::OUTPUT));
  auto component = rewriter.create<hec::ComponentOp>(funcOp.getLoc(), name,
                                                     ports, interfc, style);
  TopComp = &component;
  ...
  for (unsigned idx = 0; idx != funcOp.getNumArguments(); ++idx) {
    hec_operation[std::make_pair(funcOp, funcOp.getArgument(idx))] =
        component.getArgument(idx);
  }
  hec_operation[std::make_pair(funcOp, control_signal)] =
      component.getArgument(funcOp.getNumArguments());
  ...
  for (auto const &pair : loadstoreSet) {
    ...
    //                    std::cerr << pair.second.size() << ":" << loadsize <<
    //                    "," << storesize;
    pair.first->dump();
    llvm::SmallVector<mlir::Type, 4> types;
    std::string memInfo = "";
    for (auto const &loadOp : loadSet[pair.first]) {
      ...
      types.push_back(
          cast<tor::AllocOp>(pair.first).getType().getElementType());
      memInfo =
          "#" + std::to_string(
                    cast<tor::AllocOp>(pair.first).getType().getShape()[0]);
    }
    ...
    memSet[pair.first] =
        create_primitive(funcOp.getLoc(), types,
                         "dyn_Mem:" + std::to_string(loadsize) + "," +
                             std::to_string(storesize) + memInfo,
                         "mem_", rewriter);
    //                memOp->dump();
  }
  //            component->dump();
  ...
  for (unsigned idx = 0; idx < returnOp->getNumOperands(); ++idx) {
    ...
    //                ret.dump();
    rewriter.create<hec::AssignOp>(
        funcOp.getLoc(), component.getArgument(component.numInPorts() + idx),
    ...
    if (auto primitive = dyn_cast<hec::PrimitiveOp>(op)) {
      ...
      for (unsigned idx = 0; idx < portInfo.size(); ++idx) {
        if (portInfo[idx].direction == hec::PortDirection::OUTPUT) {
          ...
          if (use_count > 1) {
            //                                std::cerr <<
            //                                primitive.primitiveName().str() <<
            //                                "?"; primitive.dump(); std::cerr
            //                                <<
            //                                portInfo[idx].name.getValue().str()
            //                                << ":"; port.dump();
            llvm::SmallVector<mlir::Type, 4> types;
  ...
  //                    auto portInfo = primitive.getPrimitivePortInfo();
  //                    for (unsigned idx = 0; idx < portInfo.size(); ++idx) {
  //                        if (portInfo[idx].direction ==
  //                        hec::PortDirection::OUTPUT) {
  //                            auto port = primitive.getResult(idx);
  //                            if (!port.hasOneUse() && !port.use_empty()) {
  //                                std::cerr <<
  //                                portInfo[idx].name.getValue().str() <<
  //                                "!!!"; port.dump(); assert(false && "Need
  //                                Insert Fork");
  //                            }
  //                        }
  //                    }
...
void generate_top(tor::FuncOp funcOp, PatternRewriter &rewriter) {
  ...
  if (!found) {
    hecDesign = rewriter.create<hec::DesignOp>(
        funcOp.getLoc(), cast<tor::DesignOp>(funcOp->getParentOp()).symbol());
  }
...
struct DynamicSchedule : public OpRewritePattern<tor::DesignOp> {
  DynamicSchedule(MLIRContext *context)
      : OpRewritePattern<tor::DesignOp>(context, 1) {}

  LogicalResult matchAndRewrite(tor::DesignOp designOp,
    ...
    if (funcOp->hasAttr("strategy")) {
      if (auto str = llvm::dyn_cast<StringAttr>(funcOp->getAttr("strategy"))) {
...
struct DynamicSchedulePass
    : public impl::dynamicScheduleBase<DynamicSchedulePass> {
  void runOnOperation() override {
    ...
    //                        if(torFunc.getAttr("strategy").cast<StringAttr>().getValue()
    //                        == "dynamic") {
    //                            torFunc.dump();
    //                            for (auto &operation :
    //                            torFunc.getRegion(0).front()) {
    //                                operation.dump();
    //                            }
